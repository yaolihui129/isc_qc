...
class MineController extends BasicController
{
    public function index()
    {
        ...
        //预约中
        $where=array('borrower'=>$_SESSION['account'],'type'=>'1');
        ...
        //借用中
        $where=array('borrower'=>$_SESSION['account'],'type'=>'0');
        $data=$m->where($where)->order('start_time desc')->select();
        $this->assign('data0', $data);

        $data=M("tp_device_overdue")->where(array('borrower'=>$_SESSION['account'],"deleted"=>"0"))->order('end_time desc')->select();
