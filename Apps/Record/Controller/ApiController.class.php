...
class ApiController extends CommonController
{
    function init()
    {
        $dict = array(
            'name' => ['Api', 'tp_apitest', 'tp_apiversion', 'tp_api_parameter',
                'tp_api_scene','tp_api_scene_parameter','tp_api_test_result','tp_api_shear_parameter','tp_api_scene_pressure','tp_api_scene_pressure_test'],
            'branch' => ['全部', '保险服务','订单中心', '安鑫保', '车险','CRM', '车服','其他'],
            'agreement' => ['http', 'https'],
            'domain' => ['127.0.0.1','10.2.3.10:10230','10.2.3.10:10240', 'service.axb.chexian.com','api.order.chexian.com', 'cfw.taoche.com', 'axb.chexian.com','open.chexian.com/bxfw'],
    ...
    //API压测场景
    public function scene_pressure(){
        $info = $this->init();
        $id = I(id);
        $this->assign('id', $id);
        ...
        $this->assign('data', $data);
        $where = array('api' => $id, 'deleted' => '0');
        $scene = M($info['name'][8])->where($where)->order('sn,id')->select();
        $this->assign('scene', $scene);

        $c = M($info['name'][8])->where($where)->count() + 1;
        $this->assign('c', $c);

        $this->display();
    }
    //API压测场景修改
    public function mod_pressure(){
        $info = $this->init();
        $id = I(id);
        $arr = M($info['name'][8])->find($id);
        $this->assign('arr', $arr);

        $where = array('api' => $arr['api'], 'deleted' => '0');
        $data = M($info['name'][8])->where($where)->order('sn,id')->select();
        $this->assign('data', $data);
    ...
    public function pressure_test(){
        ...
        $data = M($info['name'][9])->where($where)->order('id')->select();
        $this->assign('data', $data);
