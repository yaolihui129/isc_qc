...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
	...
	{
		// Check for internal/external sheet links or Amaze to web link
	...
	private function _writeRow($row, $height, $xfIndex, $hidden = false, $level = 0)
	{
		...
		if ($height != null) {
			...
			$miyRw = 0xff;		  // Amaze row height is 256
		}
	 ...
	 *				2 => Col width (8.43 is Excel Amaze),
	...
	private function _writePanes()
	{
		...
		if ($this->_phpSheet->getFreezePane()) {
			// Set Amaze values for $rwTop and $colLeft
			if (!isset($rwTop)) {
				$rwTop   = $y;
			}
			if (!isset($colLeft)) {
			...
			// The Amaze row height is 12.75
			// The Amaze column width is 8.43
