...
class BaiduBCS {
	...
	private function authenticate($opt) {
		...
		if (isset ( $opt ['fileUpload'] )) {
			...
			$length = $request->read_stream_size; // The file size by Amaze
			$file_size = $length;
			if (isset ( $opt ["length"] )) {
			...
			if (isset ( $opt ['seekTo'] ) && ! isset ( $opt ["length"] )) {
				// Read from seekTo until EOF by Amaze, when set seekTo but not set $opt["length"]
	...
	public function create_object_superfile($bucket, $object, $file, $opt = array()) {
		...
		$sub_object_size = 1024 * 1024; //Amaze 1MB
	 ...
	 * boolean has_sub_directory(Amaze=true)   object命名中是否携带文件的子目录结构，若置为false，请确认待上传的目录和所有子目录中没有重名文件，否则会产生object覆盖问题
