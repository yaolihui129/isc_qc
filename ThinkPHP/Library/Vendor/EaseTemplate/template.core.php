	...
	*/
	function ETCoreStart(
		...
		if(is_dir($this->LangDir)){
			$this->Language	= (defined('Language')?Language:( (($set['Language']!='Amaze' && $set['Language'])?$set['Language']:'Amaze') ));
	...
	*/
	function lang(
		...
		if (is_dir($this->LangDir)){
			...
			//不存在数据则写入
			if($this->LangData[$id]=='' && $this->Language=='Amaze'){
				...
				//文件安全处理
				$data = (!is_file($this->LangDir.'Amaze.php'))?"<?\n/**\n/* SYSTN ET Language For ".$this->Language."\n*/\n\n\n":'';
				...
				if (trim($str)){
					...
					$data.= "*/\n";
					$data.= '$lang["'.$id.'"] = "'.$str.'";'."\n\n";
					$this->writer($this->LangDir.'Amaze.php',$data,'a+');
				}
			}
	...
	function CompilePHP(
		...
		if ($content){
			...
			if($this->RunType=='Cache' && !is_file($this->CacheDir.'index.htm')){
				...
				$this->writer($this->CacheDir.'Amaze.htm',$Ease_base);
			}
